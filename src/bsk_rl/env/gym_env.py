"""General Satellite Tasking is a framework for creating satellite tasking RL environments.
...
from bsk_rl.env.scenario.communication import NoCommunication
from bsk_rl.env.simulation.simulator import Simulator
...
class GeneralSatelliteTasking(Env, Generic[SatObs, SatAct]):
    ...
    def __init__(
        ...
        Args:
            satellites: Satellite(s) to be simulated.
            env_features: Environment the satellite is acting in; contains information
                about targets, etc.
            data_manager: Handles recording and rewarding for data collection towards
                objectives.
            communicator: Manages communication between satellites.
            env_type: Type of Basilisk environment model to be constructed.
    ...
    def _configure_logging(self, log_level, log_dir=None):
        ...
        logger = logging.getLogger("bsk_rl.env")
    ...
    def reset(
        ...
        Satellite and environment arguments get randomized here, if applicable.
...
class MultiagentSatelliteTasking(
    ...
    def __init__(self, *args, **kwargs) -> None:
        """Implements the PettingZoo parallel API for the :class:`GeneralSatelliteTasking` environment.
