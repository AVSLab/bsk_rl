...
class StripImagingDynModel(ImagingDynModel):
    """Equips the satellite with an instrument, storage unit, and transmitter for strip imaging."""

    def __init__(self, *args, **kwargs) -> None:
        """Equips the satellite with an instrument, storage unit, and transmitter.

        This dynamics model is used with :class:`~bsk_rl.sats.StripImagingSatellite`. It
        provides the satellite with the ability to take images of a strip target. 
        """
        super().__init__(*args, **kwargs)
    ...
    def _setup_dynamics_objects(self, **kwargs) -> None:
        ...
        self.setup_transmitter(**kwargs)
        self.setup_storage_unit(**kwargs)
        self.setup_imaging_strip(**kwargs)

    @default_args(instrumentBaudRate=8e6)
    def setup_instrument(
        self, instrumentBaudRate: float, priority: int = 895, **kwargs
    ) -> None:
        """Set up the instrument data collection model for strip imaging.
        ...
        self.instrument = simpleInstrument.SimpleInstrument()
        self.instrument.ModelTag = "instrument" + self.satellite.name
        self.instrument.nodeBaudRate = instrumentBaudRate
        self.instrument.nodeDataName = "Instrument" + self.satellite.name
        self.simulator.AddModelToTask(
            self.task_name, self.instrument, ModelPriority=priority
        )
    ...
    def setup_imaging_strip(
        ...
        **kwargs,
        ...
        """Add a generic imaging strip target to dynamics.

        The start and end strip must be updated with particular locations when used.
        ...
        self.imagingStrip.addSpacecraftToModel(self.scObject.scStateOutMsg)
