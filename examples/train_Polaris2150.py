...
if __name__ == "__main__":
    ...
    model_name = f"oct14_restrictedResources_obsv7_1e-5lr_batch5000_gamma9997_50d50i_reducedFailurePenalty.out_{N}"
    n_envs = (
        get_available_cores()  # leave some extra cores for other processes
    )
    output_dir = (
        Path("/scratch/alpine/dahu1128/oct14rllib_results").expanduser() / f"oct14_restrictedResources_obsv7_1e-5lr_batch5000_gamma9997_50d50i_reducedFailurePenalty_{time.time()}" #change this when running on cluster (add /scratch/alpine/dahu1128/rllib_results as directory)
    )
