...
# 96-hour CURC training job for BigNetwork image-only baseline derived from updated_train_Polaris.py.
# Submit from /projects/$USER/bsk_rl with:
#   sbatch examples/amos_2026/sbatch_updated_train_polaris_big_network_imaging_only_96h.sh

#SBATCH --account=ucb550_asc2
#SBATCH --job-name=polaris_big_img2_96h
#SBATCH --ntasks=1
...
export BSK_RL_WANDB_GROUP=${BSK_RL_WANDB_GROUP:-polaris-big-network-imaging-only-from-full-target-only-obs-v9-96h}
...
export BSK_RL_IMAGE_STORAGE_CAPACITY_IMAGES=${BSK_RL_IMAGE_STORAGE_CAPACITY_IMAGES:-1000}
...
echo "Running updated_train_Polaris_BigNetwork_ImagingOnly.py BigNetwork target-only obs-v9 96h training script"
python3 -u examples/updated_train_Polaris_BigNetwork_ImagingOnly.py
