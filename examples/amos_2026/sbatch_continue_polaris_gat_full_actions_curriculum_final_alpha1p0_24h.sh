...
# Continue the final checkpoint of the 0.0->1.0 curriculum policy for 24h at alpha=1.0.
# The original curriculum run is copied before restore/training, so it is not modified.
#
...
#SBATCH --job-name=gat_cont_cl_a1p0_24h
#SBATCH --ntasks=1
...
: "${BSK_RL_CONTINUE_FROM:?Set BSK_RL_CONTINUE_FROM to the final curriculum run or checkpoint directory.}"
...
export BSK_RL_WANDB_GROUP=polaris-gat-full-actions-obs-v9-curriculum-final-alpha1p0-continue24h
...
# Keep the curriculum's final task fixed: all downlink reward, no imaging reward.
export BSK_RL_DOWNLINK_BONUS=1.0
export BSK_RL_ALPHA_CURRICULUM=0
export BSK_RL_REWARD_SPLIT_TAG=curriculumFinalThen100d00i
export BSK_RL_ALPHA_TAG=curriculum0to1_then_alpha1p0
...
export BSK_RL_CONTINUE_SUFFIX=${BSK_RL_CONTINUE_SUFFIX:-continueAlpha1p0_24h_${SLURM_JOB_ID}_${SLURM_ARRAY_TASK_ID:-0}}
...
echo "Fixed continuation alpha: $BSK_RL_DOWNLINK_BONUS"
...
echo "Continuing copied curriculum-final GAT policy for 24h at fixed alpha=1.0"
