...
# Submit the mixed-regime 200-target / 45,000-second AMOS 2026 GAT MC campaign.
#
# Target catalog:
#   50% LEO, 30% MEO, 20% GEO
#
# Priority event:
#   enabled halfway through the episode, with 5 HIO targets at priority 5 and
#   3 SHIO targets at priority 10, matching the obs-v9 training setup.
#
...
#   bash examples/amos_2026/submit_gat_reward_sweep_mc_mixed_200targets_45000s_0to99.sh
#   bash examples/amos_2026/submit_gat_reward_sweep_mc_mixed_200targets_45000s_0to99.sh 5
#
# Restart behavior:
#   Re-run the same command with BSK_RL_MC_OUTPUT_ROOT pointing at the existing
#   campaign folder. Completed matching policy/seed/config runs are skipped.
...
OUTPUT_ROOT=${BSK_RL_MC_OUTPUT_ROOT:-/scratch/alpine/$USER/amos2026_mc/gat_full_actions_eval_100d00i_mixed_50LEO30MEO20GEO_${N_TARGETS}targets_${TOTAL_TIME_SEC}s_HIO${HIO_COUNT}_SHIO${SHIO_COUNT}_${CAMPAIGN_ID}}
...
for ((seed_start = START_BLOCK; seed_start <= END_BLOCK; seed_start += SEEDS_PER_BLOCK)); do
    ...
    job_name="gat_mc_mix_${N_TARGETS}t_s$(printf '%03d' "$seed_start")_$(printf '%03d' "$seed_end")"
    sbatch_args=(
        --parsable
        --job-name="$job_name"
