...
# Reproduce the October alpha-0.1 LEO-trained policy in both environments.
#
# Usage:
#   bash examples/breckenridge2026/submit_leo_baseline_mc.sh [max-concurrent-per-cell]
...
LEO_POLICY=${BRECK_MC_LEO_POLICY:-$PWD/policies/breckenridge2026_leo_trained_10d90i/checkpoint_000145}
CAMPAIGN_ID=${BRECK_MC_CAMPAIGN_ID:-$(date -u +%Y%m%dT%H%M%SZ)}
OUTPUT_ROOT=${BRECK_MC_OUTPUT_ROOT:-/scratch/alpine/$USER/breckenridge2026_mc/leo_trained_baseline_10d90i_${CAMPAIGN_ID}}
MANIFEST=${BRECK_MC_MANIFEST:-$OUTPUT_ROOT/breckenridge2026_leo_baseline_manifest.json}
...
if [[ ! -f "$LEO_POLICY/learner_group/learner/rl_module/inspector/module_state.pt" ]]; then
    echo "Bundled LEO-trained policy is missing or incomplete: $LEO_POLICY" >&2
    exit 1
fi

echo "Checking the bundled October policy with the Alpine virtual environment..."
"$VENV_PYTHON" - "$LEO_POLICY" <<'PY'
...
echo
echo "Submitting two independent October-policy reproduction arrays"
