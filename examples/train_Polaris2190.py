...
if __name__ == "__main__":
    ...
    sat_args["storedCharge_Init"] = lambda: np.random.uniform(0.10, 0.5) * 500 * 3600 #*1000000
    ...
    model_name = f"oct14_restrictedResources_obsv7_1e-5lr_batch5000_gamma9997_90d10i_reducedFailurePenalty.out_{N}"
    n_envs = (
        get_available_cores()  # leave some extra cores for other processes
    )
    output_dir = (
        Path("/scratch/alpine/dahu1128/oct14rllib_results").expanduser() / f"oct14_restrictedResources_obsv7_1e-5lr_batch5000_gamma9997_90d10i_reducedFailurePenalty_{time.time()}" #change this when running on cluster (add /scratch/alpine/dahu1128/rllib_results as directory)
    )
