...
if __name__ == "__main__":
    ...
    sat_args["low_battery_penalty"] = -0.5
    ...
    model_name = f"oct14_restrictedResources_obsv7_1e-5lr_batch5000_gamma9997_100d00i_reducedFailurePenalty_lowBatPenalty.out_{N}"
    n_envs = (
        get_available_cores()  # leave some extra cores for other processes
    )
    output_dir = (
        Path("/scratch/alpine/dahu1128/oct14rllib_results").expanduser() / f"oct14_restrictedResources_obsv7_1e-5lr_batch5000_gamma9997_100d00i_reducedFailurePenalty_lowBatPenalty_{time.time()}" #change this when running on cluster (add /scratch/alpine/dahu1128/rllib_results as directory)
    )
