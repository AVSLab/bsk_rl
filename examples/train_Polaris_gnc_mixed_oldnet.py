...
if __name__ == "__main__":
    ...
    def reward_label(downlink_bonus):
        downlink_percent = int(round(downlink_bonus * 100))
        imaging_percent = 100 - downlink_percent
        return f"{downlink_percent}d{imaging_percent:02d}i"

    def mix_label(weights):
        normalized = {key: weights.get(key, 0.0) for key in ("LEO", "MEO", "GEO")}
        total = sum(normalized.values())
        if total <= 0:
            raise ValueError("Mixed-regime weights must sum to a positive value.")
        return "L{LEO:02.0f}M{MEO:02.0f}G{GEO:02.0f}".format(
            **{key: 100 * value / total for key, value in normalized.items()}
        )

    user = os.environ.get("USER", "dahu1128")
    parser = argparse.ArgumentParser(
        description="Train old-network GNC PPO policies on a mixed LEO/MEO/GEO target catalog."
    )
    parser.add_argument("--downlink-bonus", type=float, default=0.0)
    parser.add_argument("--mix-weights", default="LEO=0.5,MEO=0.3,GEO=0.2")
    parser.add_argument("--run-prefix", default="gnc_MIXED_oldnet")
    parser.add_argument("--total-timesteps", type=int, default=20_000_000)
    parser.add_argument("--batch-multiplier", type=int, default=350)
    parser.add_argument("--checkpoint-frequency", type=int, default=3)
    ...
    parser.add_argument(
        "--output-root",
        default=f"/scratch/alpine/{user}/rllib_results",
    )
    parser.add_argument("--temp-dir", default=f"/scratch/alpine/{user}/tmp")
    ...
    model_name = f"{args.run_prefix}_{batch_size}batch_mix{target_mix_label}_restrictedResources_obsv7_1e-5lr_0.05cp_gradclip0.5_gamma9997_{reward_mix_label}.out_{N}"

    output_dir = (
        Path(args.output_root).expanduser() / f"{args.run_prefix}_{batch_size}batch_mix{target_mix_label}_restrictedResources_obsv7_1e-5lr_0.05cp_gradclip0.5_gamma9997_{reward_mix_label}_{time.time()}"
    )
    ...
    with open(output_dir / f"{model_name}_params_gnc_mixed_oldnet.txt", "w") as file:
        yaml.dump(sanitize_np(job_args), file)
    with open(output_dir / f"{model_name}_run_config_gnc_mixed_oldnet.yaml", "w") as file:
        yaml.dump(sanitize_np(run_cfg), file)

    train_model(
        model_name=model_name,
        output_directory=output_dir,
        checkpoint_frequency=args.checkpoint_frequency, # used to be 2
