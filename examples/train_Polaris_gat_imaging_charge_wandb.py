#!/usr/bin/env python3
"""AMOS 2026 Polaris GAT trainer with imaging plus charge actions.
...
Local terminal equivalent:
    /Users/dahu1128/Repositories/bsk_rl/.venv/bin/python \
        /Users/dahu1128/Repositories/bsk_rl/examples/train_Polaris_gat_imaging_charge_wandb.py

Cluster use:
    Submit the matching Slurm wrapper, not the Python file directly:
        sbatch examples/amos_2026/sbatch_train_polaris_gat_imaging_charge_wandb_24h.sh
...
This script intentionally exposes only charge and image actions. Because there
is no downlink action in this ablation, image quality reward is paid at capture
time here only; the normal AMOS pending/downlink-verification lifecycle remains
in the baseline `examples/updated_train_Polaris.py` path.

This is observation layout v9 for the GAT imaging+charge policy: one satellite
...
OBS_SAT_DIM = 10  # storage, battery, wheel_speeds(3), s_hat_H(3), eclipse start/end
...
NON_IMAGING_ACTIONS = 1  # action 0 is charge; image actions start at action 1
    ...
    try:
        return WandbLogger(
            ...
            group=os.environ.get(
                "BSK_RL_WANDB_GROUP", "polaris-gat-imaging-charge-obs-v9"
            ),
            key_path=key_path,
            config=config,
        )
...
def gat_model_config(n_targets_ahead: int) -> dict[str, Any]:
    """Return the GAT settings for one charge action plus target image actions."""
    return {
...
def env_metrics_callback(env):
    """Episode-level metrics useful for the imaging+charge GAT experiment."""
    data = {}
    ...
    # These stay at zero in this no-downlink/no-desat ablation, which makes
...
if __name__ == "__main__":
    ...
    sim_cfg = SimConfig(
        ...
        just_imaging=True,  # no downlink/desat; this variant adds charge back to the image actions
    ...
    def make_rso_rewarder():
        return data.RSOTargetImageReward(
            ...
            # capture time or the imaging+charge policy would never receive reward.
    ...
    class MyScanningSatellite(sats.AccessSatellite):
        ...
        action_spec = [
            # GATModule expects non-imaging logits first, so action 0 is charge.
    ...
    # No downlink exists in this ablation, so storage is intentionally generous.
    # Battery uses the normal AMOS sizing so the extra charge action matters.
    image_bits = 8e6 / 2
    ...
    run_tag = (
        f"amos2026_LEO_GAT_imagingCharge_obs-v9_{batch_size}batch_"
