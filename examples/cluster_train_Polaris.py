...
if __name__ == "__main__":
    ...
    model_name = f"1e-5lr_0005torque_{sat_args["downlink_bonus"]*100}d{sat_args["imaging_bonus"]*100}i_reward_new_penalties_smallest_storage_{N}"
    n_envs = (
        get_available_cores() - 4  # leave some extra cores for other processes
    )
    output_dir = (
        Path("/scratch/alpine/dahu1128/july10rllib_results").expanduser() / f"july10_1e-5lr_0005torque_{sat_args["downlink_bonus"]*100}d{sat_args["imaging_bonus"]*100}i_reward_new_penalties_smallest_storage_Polaris_simulation_{time.time()}" #change this when running on cluster (add /scratch/alpine/dahu1128/rllib_results as directory)
    )
    ...
    with open(output_dir / f"{model_name}_params_july10.txt", "w") as file: # update this when running on cluster
