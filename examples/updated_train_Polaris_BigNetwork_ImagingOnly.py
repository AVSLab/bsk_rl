...
if __name__ == "__main__":
    ...
    sim_cfg = SimConfig(
        ...
        obs_v=9.0,
        just_imaging=True,
        # Image-only has no downlink action, so useful images are credited at capture.
    ...
    class MyScanningSatellite(sats.AccessSatellite):
        observation_spec = [
            # Image-only BigNetwork baseline: keep only target-selection features.
    ...
    # Pure image reward: there is no downlink action in this baseline.
    ...
    run_tag = (
        f"amos2026_LEO_wGAE_BigNetwork_ImagingOnlyFromFull_00d100i_{batch_size}batch_"
        "targetOnly_obs-v9_captureReward_1e-5lr_0.05cp_gradclip0.5_gamma9997"
    )
    ...
    if os.environ.get("SLURM_JOB_ID"):
        ...
        output_root = Path(
            os.environ.get(
                "BSK_RL_OUTPUT_DIR",
                "~/rllib_results/may_results/may19rllib_results",
            )
        ).expanduser()
        ray_tmpdir = Path(os.environ.get("BSK_RL_RAY_TMPDIR", f"/tmp/bskrl_{N}"))

    output_dir = output_root / f"may19_BigNetwork_ImagingOnlyFromFull_00d100i_{run_tag}_{time.time()}"
    ...
    run_cfg = {
        "sim": asdict(sim_cfg),
        "model_family": "big_network_fc_imaging_only_from_full",
        "observation_layout": {
            "obs_v": sim_cfg.obs_v,
            "target_only": True,
            "blocks": [
        ...
        "wandb": {
            ...
            "group": os.environ.get(
                "BSK_RL_WANDB_GROUP",
                "polaris-big-network-imaging-only-from-full-target-only-obs-v9",
            ),
        },
