...
if __name__ == "__main__":
    ...
    sat_args["storedCharge_Init"] = lambda: np.random.uniform(0.25, 0.5) * 500 * 3600
    ...
    model_name = f"aug18_restrictedResources_obsv21_1e-3lr_0.1cp_gamma9995_100d0i.out_{N}"
    n_envs = (
        get_available_cores() - 8  # leave some extra cores for other processes
    )
    output_dir = (
        Path("~/rllib_results/july_results/july30rllib_results").expanduser() / f"aug18_restrictedResources_obsv21_1e-3lr_0.1cp_gamma9995_100d0i_{time.time()}" #change this when running on cluster (add /scratch/alpine/dahu1128/rllib_results as directory)
    )
