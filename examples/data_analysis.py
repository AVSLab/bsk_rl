...
for i in range(len(inspector_sigmaBN)):
    ...
    alignment_values.append(error_angle)  # Store error angle instead of dot product
...
plt.savefig("plots/pointing_error.pdf", format="pdf")  # Save as PDF
plt.show()
...
cmap = cm.get_cmap("inferno")  # or use plasma or inferno
...
if target_orbit == 'GEO':
    ...
    arrow_scale = 2e6
c_target = "purple"
ax.scatter(target_r_BN_N[0, 0], target_r_BN_N[0, 1], target_r_BN_N[0, 2], label=str("Target Trajectory ~"+str(step)+" min interval"), color=c_target,marker='x')
...
for i in range(0, int(len(inspector_r_BN_N)/1), step):
    ...
    # Target position
    ax.scatter(target_r_BN_N[i, 0], target_r_BN_N[i, 1], target_r_BN_N[i, 2], color=c_target,marker='x')
