...
class PolicyWrapper:
    def __init__(self, policy_path, policy_mode='latest', zero_element=None):
...
def save_policy_plots(policy_name, agg, per_run_arrays, outdir: Path, tag: str):
    ...
    figpath = outdir / f"{policy_name}__seed_{SEED_START}__mode_{heuristic_mode}__{tag}__action_fourbar.png"
    ...
    if all_battery and all_storage:
        ...
        ax.set_ylim(0, 1.05)
        ax.legend()
        fig2path = outdir / f"{policy_name}__seed_{SEED_START}__mode_{heuristic_mode}__{tag}__battery_storage_avg.png"
...
def main():
    ...
    for p in policies:
        ...
        policy_name = p.name
        safe_name = "heuristic_policy" # slugify(policy_name)
        ...
        if just_imaging:
            outfolder = SAVE_DIR / f"JustImaging__seed{SEED_START}_{safe_name}__mode_{heuristic_mode}__{batch_tag}"
        else:
            outfolder = SAVE_DIR / f"seed{SEED_START}_{safe_name}__mode_{heuristic_mode}__{batch_tag}"
        outfolder.mkdir(parents=True, exist_ok=True)

        # include the tag in filenames too
        summary_path = outfolder / f"seed{SEED_START}__{safe_name}__mode_{heuristic_mode}__{batch_tag}__results_summary.json"
        ...
        if just_imaging:
            npz_path = outfolder / f"JustImaging_seed_{SEED_START}__{safe_name}__mode_{heuristic_mode}__{batch_tag}__{shield_tag}__rundata.npz"
        else:
            npz_path = outfolder / f"seed_{SEED_START}__{safe_name}__mode_{heuristic_mode}__{batch_tag}__{shield_tag}__rundata.npz"
