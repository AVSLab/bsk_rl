...
requested_policy_name = args.policy_name or "oct14_obsv7_1e_5lr_batch5000_gamma9997_10d90i"
if use_heuristic:
...
# Ground-station window observations use normalized offsets in this study.
GS_NORMALIZED = True

# Bundled publication policies
REPO_ROOT = Path(__file__).resolve().parents[1]
BUNDLED_ALPHA_POLICIES = {
    "oct14_obsv7_1e_5lr_batch5000_gamma9997_0d100i": REPO_ROOT
    / "policies"
    / "breckenridge2026_alpha_sweep"
    / "0d100i",
    "oct14_obsv7_1e_5lr_batch5000_gamma9997_10d90i": REPO_ROOT
    / "policies"
    / "breckenridge2026_alpha_sweep"
    / "10d90i",
    "oct14_obsv7_1e_5lr_batch5000_gamma9997_20d80i": REPO_ROOT
    / "policies"
    / "breckenridge2026_alpha_sweep"
    / "20d80i",
    "oct14_obsv7_1e_5lr_batch5000_gamma9997_30d70i": REPO_ROOT
    / "policies"
    / "breckenridge2026_alpha_sweep"
    / "30d70i",
    "oct14_obsv7_1e_5lr_batch5000_gamma9997_40d60i": REPO_ROOT
    / "policies"
    / "breckenridge2026_alpha_sweep"
    / "40d60i",
    "oct14_obsv7_1e_5lr_batch5000_gamma9997_50d50i": REPO_ROOT
    / "policies"
    / "breckenridge2026_alpha_sweep"
    / "50d50i",
    "oct14_obsv7_1e_5lr_batch5000_gamma9997_60d40i": REPO_ROOT
    / "policies"
    / "breckenridge2026_alpha_sweep"
    / "60d40i",
    "oct14_obsv7_1e_5lr_batch5000_gamma9997_70d30i": REPO_ROOT
    / "policies"
    / "breckenridge2026_alpha_sweep"
    / "70d30i",
    "oct14_obsv7_1e_5lr_batch5000_gamma9997_80d20i": REPO_ROOT
    / "policies"
    / "breckenridge2026_alpha_sweep"
    / "80d20i",
    "oct14_obsv7_1e_5lr_batch5000_gamma9997_90d10i": REPO_ROOT
    / "policies"
    / "breckenridge2026_alpha_sweep"
    / "90d10i",
    "oct14_obsv7_1e_5lr_batch5000_gamma9997_100d00i": REPO_ROOT
    / "policies"
    / "breckenridge2026_alpha_sweep"
    / "100d00i",
}
...
def latest_checkpoint(policy_dir: Path) -> Path:
    checkpoints = sorted(
        policy_dir.glob("checkpoint_*"),
        key=lambda p: int(p.name.split("_")[-1]),
    )
    if not checkpoints:
        raise FileNotFoundError(f"No checkpoint_* directories found under {policy_dir}")
...
elif policy_name in BUNDLED_ALPHA_POLICIES:
    policy_path = str(latest_checkpoint(BUNDLED_ALPHA_POLICIES[policy_name]))
else:
    available = ", ".join(sorted(BUNDLED_ALPHA_POLICIES))
    raise ValueError(
        f"Unknown bundled policy '{policy_name}'. "
        f"Use --policy_path or choose one of: {available}"
    )

policy_obs_map = {name: 7 for name in BUNDLED_ALPHA_POLICIES}
policy_obs_map.update(
    {
        "breckenridge2026_leo_trained_10d90i": 7,
        "breckenridge2026_mixed_trained_10d90i": 7,
    }
)
...
def custom_oe_randomizer(regime: str = "LEO",
    ...
    """
    Zero-argument sampler of ClassicElements.
    - Called with no args -> defaults to LEO.
    - For MEO/GEO or mixed, wrap with a zero-arg lambda/partial that sets 'regime'.

    Example:
        oe = custom_oe_randomizer()                       # LEO default
...
# Command times from the ImageRSO action specification.
...
imaging_window = float(imaging_duration)
...
# fig3, ax1 = plt.subplots(figsize=(12, 6))
#
# # --- shading for umbra and ground-station windows ---
#
...
# plt.show()
#
# minute = 1.0  # seconds per "minute" on the x-axis
# imaging_times = np.array(SS1_actions_spec.imaging_times)
# azimuths      = np.array(SS1_actions_spec.chosen_target_azimuth)
# elevations    = np.array(SS1_actions_spec.chosen_target_elevation_angle)
#
