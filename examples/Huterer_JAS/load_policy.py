...
def find_latest_checkpoint(checkpoint_path_dir: Path, mode: str = "latest") -> Path:
    ...
    # Use the named RLlib best checkpoint when present. If it is absent, fall back
    # to the oldest numeric checkpoint because some archived runs were renamed.
    if mode == "best":
    ...
    for path_str in all_checkpoints:
        ...
        try:
            ...
            # Skip non-numeric folders like 'checkpoint_best'.
            continue
...
def load_policy(policy_path_general: Path, policy_mode) -> Callable:
    ...
    def policy(
        ...
        Args:
            ...
            deterministic: If True, use argmax for action selection; otherwise,
