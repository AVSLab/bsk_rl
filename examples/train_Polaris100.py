...
if __name__ == "__main__":
    ...
    batch_multiplier = 350
    batch_size = int(batch_multiplier * n_envs)
    print('n_envs', n_envs, 'therefore batch size is ', batch_multiplier*n_envs)
    model_name = f"jan26_MIXED_wGAE_{batch_size}batch_restrictedResources_obsv7_1e-5lr_0.05cp_gradclip0.5_gamma9997_0d100i.out_{N}"

    output_dir = (
        Path("/scratch/alpine/dahu1128/rllib_results").expanduser() / f"jan26_MIXED_wGAE_{batch_size}batch_restrictedResources_obsv7_1e-5lr_0.05cp_gradclip0.5_gamma9997_0d100i_{time.time()}" #change this when running on cluster (add /scratch/alpine/dahu1128/rllib_results as directory)
    )
    ...
    with open(output_dir / f"{model_name}_params_jan26.txt", "w") as file: # update this when running on cluster
