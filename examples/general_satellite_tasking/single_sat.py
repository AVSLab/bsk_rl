...
# Satellite configuration arguments are inferred from the satellite type. The function
# default_sat_args collects all of the parameters that must be set for FSW and dynamics
# in the Basilisk simulation. Any parameters that are to be overridden can be set as
# arguments to default_sat_args, and an error will be raised if the parameter is not
# valid for the satellite type.

sat_args = sat_type.default_sat_args(
    imageAttErrorRequirement=0.01,  # Change a default parameter
    imageRateErrorRequirement=0.01,
    # Parameters can also be set as a function that is called each time the environment
    # is reset
    oe=random_orbit,
)
print(sat_args)
...
env = gym.make(
    # The SingleSatelliteTasking environment takes actions and observations directly
    # from the satellite, instead of wrapping them in a tuple
