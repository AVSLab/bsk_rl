#!/usr/bin/env python3
"""Local publication training script for LEO-to-any targets.

This script keeps the policy architecture and environment settings used for the
LEO-only alpha sweep and changes the target catalog from LEO-only to the mixed
LEO/MEO/GEO distribution used in the paper Monte Carlo study.
"""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Train the publication PPO policy locally on mixed LEO/MEO/GEO "
            "targets."
        )
    )
    ...
    parser.add_argument(
        "--output-root",
        default="~/rllib_results/breckenridge2026_leo_any_publication",
    )
    parser.add_argument(
        "--temp-dir",
        default=None,
    ...
    parser.add_argument(
        "--run-prefix",
        default="breckenridge2026_LEOAny_publication",
    )
    parser.add_argument(
        "--smoke-test",
        action="store_true",
...
def main() -> None:
    ...
    run_cfg = {
        ...
        "notes": (
            "Publication setup: fixed 300 s image/downlink actions, uniform "
            "target priorities, 2048x2048 inspector network, no fast actions, "
            "and no high-interest/short-notice target classes."
        ),
