...
This is observation layout v9 for the image-only Target-GNN: repeated target
chunks only. The resource and ground-station state is intentionally omitted so
the architecture focuses on target ranking in this ablation.
"""
...
# Observation bookkeeping for Target-GNN's module. The image-only experiment uses
# only repeated target chunks: [target_0 block][target_1 block]...
OBS_SAT_DIM = 0
    ...
    try:
        return WandbLogger(
            ...
            group=os.environ.get(
                "BSK_RL_WANDB_GROUP", "polaris-target-gnn-imaging-only-obs-v9"
            ),
            key_path=key_path,
            config=config,
        )
...
if __name__ == "__main__":
    ...
    sim_cfg = SimConfig(
        ...
        obs_v=9.0,  # observation layout version; v9 adds priority and rel-vel-H target features
    ...
    class MyScanningSatellite(sats.AccessSatellite):
        observation_spec = [
            # Target candidate blocks. The GNN applies the same encoder to each
            # of these n_targets_ahead chunks, then scores each image action.
    ...
    run_tag = (
        f"amos2026_LEO_targetGNN_imagingOnly_obs-v9_{batch_size}batch_"
