...
if __name__ == "__main__":
    ...
    model_name = f"aug18_restrictedResources_obsv7_5e-7lr_batch320_gamma9995_0d100i.out_{N}"
    n_envs = (
        get_available_cores()  # leave some extra cores for other processes
    )
    output_dir = (
        Path("/scratch/alpine/dahu1128/aug18rllib_results").expanduser() / f"aug18_restrictedResources_obsv7_5e-7lr_batch320_gamma9995_0d100i_{time.time()}" #change this when running on cluster (add /scratch/alpine/dahu1128/rllib_results as directory)
    )
    ...
    jobs = build_job_array(
        training_args=dict(
            lr=[5e-7],
            gamma=[0.9995],
            train_batch_size=[int(10 * n_envs)],
            num_sgd_iter=[10],
            lambda_=[0.95],
            use_kl_loss=[False],
            clip_param=[0.1],
