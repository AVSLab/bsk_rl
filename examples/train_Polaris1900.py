...
if __name__ == "__main__":
    ...
    model_name = f"aug18_restrictedResources_obsv7_1e-6lr_batch3200_gradclip0.001_cp0.005_gamma997_0d100i.out_{N}"
    n_envs = (
        get_available_cores()  # leave some extra cores for other processes
    )
    output_dir = (
        Path("/scratch/alpine/dahu1128/aug18rllib_results").expanduser() / f"aug18_restrictedResources_obsv7_1e-6lr_batch3200_gradclip0.001_cp0.005_gamma997_0d100i_{time.time()}" #change this when running on cluster (add /scratch/alpine/dahu1128/rllib_results as directory)
    )
