...
class FSWModel:
    ...
    def __init__(self, SimBase, fswRate, spacecraftIndex):
        ...
        self.mrpSteeringControlData = mrpSteering.mrpSteeringConfig()
        self.mrpSteeringControlWrap = SimBase.setModelDataWrap(
            self.mrpSteeringControlData
        )
        self.mrpSteeringControlWrap.ModelTag = "mrpSteeringControl"

        # Rate servo
        self.servoData = rateServoFullNonlinear.rateServoFullNonlinearConfig()
        self.servoWrap = SimBase.setModelDataWrap(self.servoData)
    ...
    def SetMRPSteeringRWA(self, SimBase):
        """
        Defines the control properties.
        """
        self.mrpSteeringControlData.guidInMsg.subscribeTo(self.attGuidMsg)
        self.mrpSteeringControlData.K1 = SimBase.initial_conditions[
            str(self.spacecraftIndex)
        ].get("K1")
        self.mrpSteeringControlData.K3 = SimBase.initial_conditions[
            str(self.spacecraftIndex)
        ].get("K3")
        self.mrpSteeringControlData.omega_max = SimBase.initial_conditions[
            str(self.spacecraftIndex)
        ].get("omega_max")
        self.mrpSteeringControlData.ignoreOuterLoopFeedforward = False

        self.servoData.Ki = SimBase.initial_conditions[str(self.spacecraftIndex)].get(
            "servo_Ki"
        )
        self.servoData.P = SimBase.initial_conditions[str(self.spacecraftIndex)].get(
            "servo_P"
        )
        self.servoData.integralLimit = 2.0 / self.servoData.Ki * 0.1
        self.servoData.knownTorquePntB_B = [0.0, 0.0, 0.0]
        self.servoData.guidInMsg.subscribeTo(self.attGuidMsg)
        self.servoData.vehConfigInMsg.subscribeTo(self.vcConfigMsg)
        self.servoData.rwParamsInMsg.subscribeTo(self.fswRwConfigMsg)
    ...
    def InitAllFSWObjects(self, SimBase):
        ...
        self.SetMRPSteeringRWA(SimBase)
