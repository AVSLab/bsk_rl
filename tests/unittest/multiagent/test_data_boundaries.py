...
def test_physical_stores_are_independent_and_enforce_downlink_ownership():
    store_0 = SensorProductStore("sensor_0")
    store_1 = SensorProductStore("sensor_1")
    ...
    with pytest.raises(KeyError):
        store_1.downlink("p0", 100.0)
    with pytest.raises(ValueError, match="owned"):
        store_1.store(item)
    assert store_0.downlink("p0", 100.0).delivery_time == 100.0
...
def test_global_ledger_does_not_leak_into_local_catalogs():
    ...
    ledger = TeamServiceLedger(cooldown_s=100.0, quality_threshold=0.5)
...
def test_candidate_eligibility_uses_local_catalog_not_team_truth():
    ...
    satellite = type(
        ...
        (),
        {
            ...
            "data_store": type("Store", (), {"data": local_data})(),
    ...
    team_ledger = TeamServiceLedger(cooldown_s=100.0, quality_threshold=0.5)
