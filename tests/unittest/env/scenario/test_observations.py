...
class TestObservationBuilder:
    def test_init(self):
        ...
        ob = obs.ObservationBuilder(
            satellite=MagicMock(observation_spec=observation_spec)
        )
    ...
    def make_mocked_observation(self, obs_type=np.ndarray):
        ...
        ob = obs.ObservationBuilder(sat, obs_type=obs_type)
...
class TestOpportunityProperties:
    def test_fns(self):
        ...
        assert fns["location"](sat, opp) == 1.0
    ...
    def test_target_angle(self):
        ...
        opp = dict(location=np.array([0.0, 0.0, 0.0]))
        assert np.isclose(obs._target_angle(sat, opp), np.pi / 2)

    def test_init(self):
        ob = obs.OpportunityProperties(
            dict(prop="location", norm=2.0),
            dict(
