...
from bsk_rl.env.simulation.dynamics import LOSCommDynModel
...
class TestLOSCommunication:
    ...
    @patch(
        "bsk_rl.env.scenario.communication.CommunicationMethod.communicate",
        MagicMock(),
    )
    def test_communicate(self):
        mock_sats = [MagicMock(dyn_type=LOSCommDynModel) for i in range(2)]
...
class TestMultiDegreeCommunication:
    @patch("bsk_rl.env.scenario.communication.CommunicationMethod._communication_pairs")
